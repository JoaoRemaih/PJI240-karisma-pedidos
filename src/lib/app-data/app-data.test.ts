...
describe("redirectToLoginIfRequired", () => {
    ...
    const did = withWindow(
      ...
      () =>
        redirectToLoginIfRequired({
          ...
          data: null,
          loginRequired: true,
          loginUrl: "http://localhost:8080/__gate/signin?return_to=x",
        }),
    );
    assert.equal(did, true);
    assert.equal(target, "http://localhost:8080/__gate/signin?return_to=x");
  });
  ...
  it("opens a new tab instead of navigating when framed", () => {
    ...
    assert.equal(did, true);
    assert.equal(opened, "http://localhost:8080/__gate/signin?return_to=x");
  ...
  it("falls back to navigation when framed and the popup is blocked", () => {
    ...
    assert.equal(did, true);
    assert.equal(assigned, "http://localhost:8080/__gate/signin?return_to=x");
  });
