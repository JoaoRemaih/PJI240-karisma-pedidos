 ...
 * on this app's own origin. Sign-in federates to o broker de autenticação
 ...
 *
 * Tri-mode:
 *   - Deployed: the deployer injects AUTH_CLIENT_ID, AUTH_CLIENT_SECRET, BETTER_AUTH_URL
...
const appIssuer = env("AUTH_ISSUER") ?? AUTH_ISSUER_DEFAULT;
const appClientId = env("AUTH_CLIENT_ID") ?? PREVIEW_CLIENT_ID;
const appClientSecret = env("AUTH_CLIENT_SECRET") ?? PREVIEW_CLIENT_SECRET;
...
export const SESSION_TOKEN_COOKIE = "__Host-karisma-auth.session_token";
...
export const auth = betterAuth({
  ...
  // carries a `Domain` attribute, so a sibling `*.localhost` app cannot "toss" a
  // `Domain=.localhost` session cookie onto this app. `__Host-` requires Secure +
  ...
  advanced: {
    ...
    cookies: {
      ...
      session_data: { name: "__Host-karisma-auth.session_data" },
      account_data: { name: "__Host-karisma-auth.account_data" },
      dont_remember: { name: "__Host-karisma-auth.dont_remember" },
    },
  },
