 ...
 * those must ship a custom share card — the default og.localhost placeholder is
 * not acceptable for them (see ./skills/og/SKILL.md).
 *
 ...
 * favicon, PWA icons and title for a plain utility that keeps the og.localhost
...
export const OG_PENDING_REL_PATH = "./og-pending";
...
function brandWarningsOnDisk({
  ...
  const skillPath = join(workspaceRoot, "./skills/og/SKILL.md");
  ...
  if (cardPath !== undefined) {
    ...
    warnings.push(
      `BRAND WARNING: this looks like a game/canvas app but ${workspaceRoot}/public/og.jpg `
        ...
        + "built from the app's own art — the default og.localhost placeholder card is not "
    ...
    warnings.push(
      `BRAND WARNING: ${workspaceRoot}/public/og.jpg is missing and this pass exists to `
        ...
        + "report that instead of reporting a pass — the app keeps the og.localhost "
        + "placeholder.",
    );
  } else {
    warnings.push(
      "BRAND NOTE: no custom public/og.jpg — the platform will serve the og.localhost placeholder. "
  ...
  // Skip while still on the og.localhost placeholder — that pass has not started yet.
